...
export default class Search {
  ...
  events() {
    ...
    this.headerSearchIcon.addEventListener("click", (error) => {
  ...
  keyPressHandler() {
    ...
    if (value != '' && value != this.previousValue) {
      ...
      this.showLoaderIcon();
      this.hideResultsArea();
      this.typingTimer = setTimeout(() => this.sendRequest(), 500);
    };
  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      ${posts.map(post => {
        ...
        return `<a href="/post/${post.slug}" class="list-group-item list-group-item-action">
